...
fn check_dump_ast_matches_float_literal_contract() {
    ...
    fs::create_dir_all(&examples_dir).expect("fixture setup should create examples directory");
    fs::write(
        examples_dir.join("float_literals.tn"),
        "defmodule Primitives do\n  def run() do\n    tuple(3.14, 0.5)\n  end\nend\n",
    )
    .expect("fixture setup should write float literal source file");

    let output = std::process::Command::new(env!("CARGO_BIN_EXE_tonic"))
        .current_dir(&fixture_root)
        .args(["check", "examples/float_literals.tn", "--dump-ast"])
        .output()
        .expect("check command should run");
...
fn check_dump_ast_matches_heredoc_literal_contract() {
    ...
    fs::create_dir_all(&examples_dir).expect("fixture setup should create examples directory");
    fs::write(
        examples_dir.join("heredoc_literal.tn"),
        "defmodule Primitives do\n  def run() do\n    \"\"\"hello\nworld\"\"\"\n  end\nend\n",
    )
    .expect("fixture setup should write heredoc source file");

    let output = std::process::Command::new(env!("CARGO_BIN_EXE_tonic"))
        .current_dir(&fixture_root)
        .args(["check", "examples/heredoc_literal.tn", "--dump-ast"])
        .output()
        .expect("check command should run");
...
#[test]
fn check_dump_ast_matches_text_block_literal_contract() {
    let fixture_root = common::unique_fixture_root("check-dump-ast-text-block");
    let examples_dir = fixture_root.join("examples");

    fs::create_dir_all(&examples_dir).expect("fixture setup should create examples directory");
    fs::write(
        examples_dir.join("text_block_literal.tn"),
        "defmodule Primitives do\n  def run() do\n    ~t\"\"\"\n      hello\n        world\n      done\n    \"\"\"\n  end\nend\n",
    )
    .expect("fixture setup should write text-block source file");

    let output = std::process::Command::new(env!("CARGO_BIN_EXE_tonic"))
        .current_dir(&fixture_root)
        .args(["check", "examples/text_block_literal.tn", "--dump-ast"])
        .output()
        .expect("check command should run");
...
fn check_dump_ast_supports_unary_plus_and_minus() {
    ...
    .expect("fixture setup should write unary expression source file");

    let output = std::process::Command::new(env!("CARGO_BIN_EXE_tonic"))
        .current_dir(&fixture_root)
        .args(["check", "examples/unary_plus_minus.tn", "--dump-ast"])
        .output()
        .expect("check command should run");
    ...
    let expected = concat!(
        ...
        "{\"name\":\"compute\",\"params\":[],\"body\":{",
        "\"kind\":\"binary\",\"op\":\"plus\",",
        "\"left\":{\"kind\":\"unary\",\"op\":\"minus\",",
        "\"value\":{\"kind\":\"int\",\"value\":1}},",
        "\"right\":{\"kind\":\"unary\",\"op\":\"plus\",",
